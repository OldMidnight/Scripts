	...
	case "$INPUT" in
		"$INPUT")
			...
			then
				...
				if [ ! -d /sdcard/'MidnightMain(Beta)'/MidnightFonts/MidnightMake ]
				then
					mkdir /sdcard/'MidnightMain(Beta)'/MidnightFonts/MidnightMake
					if [ $? == 0 ]
					then
				...
				wget -q -O /sdcard/'MidnightMain(Beta)'/MidnightFonts/MidnightMake/midmake_beta.sh "$LINK"
				if [ $? == 0 ]
				then
				...
				sh /sdcard/'MidnightMain(Beta)'/MidnightFonts/MidnightMake/midmake_beta.sh
				if [ $? == 0 ]
				then
				...
				rm -f /sdcard/'MidnightMain(Beta)'/MidnightFonts/MidnightMake/midmake_beta.sh
				main=$((main + 1))
			elif [ "$INPUT" == "q" ]
			then
				if [ -d "$STOREFONT" ]
				...
				LINK="$( cat /sdcard/'MidnightMain(Beta)'/MidnightFonts/tmp_fonts/DONT-DELETE-2 | xargs | tr -s " " | cut -d " " -f $INPUT )"
				FONT="$( cat /sdcard/'MidnightMain(Beta)'/MidnightFonts/tmp_fonts/DONT-DELETE | xargs | cut -d "." -f $INPUT )"
